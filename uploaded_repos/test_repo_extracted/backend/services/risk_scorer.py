...
def generate_training_data(n_samples: int = 2000):
    ...
    loc               = np.random.randint(10, 1500, n_samples).astype(float)
    cc_avg            = np.random.uniform(1, 30, n_samples)
    cc_max            = cc_avg + np.random.uniform(0, 15, n_samples)
    halstead_effort   = np.random.uniform(0, 50000, n_samples)
    maintainability   = np.random.uniform(0, 100, n_samples)
    churn_rate        = np.random.uniform(0, 50, n_samples)

    # Risk score formula based on SE research
    risk = (
        0.30 * (cc_avg / 30) +
        0.20 * (cc_max / 45) +
        0.15 * (churn_rate / 50) +
        0.15 * (loc / 1500) +
        0.10 * (halstead_effort / 50000) +
...
def compute_hotspot_score(cc_avg: float, churn_rate: float, loc: int,
                           max_cc: float = 30, max_churn: float = 50, max_loc: float = 1500) -> float:
...
def compute_tdc(risk_score: float, hourly_rate: float = 75.0) -> float:
    ...
    # Remediation hours based on risk tier
    if risk_score >= 75:
